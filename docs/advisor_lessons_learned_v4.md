...
### Why this matters for the mod
A good Advisor doesn't speculate — it retrieves the relevant data first, then speaks. Confident wrong answers are more damaging than honest uncertainty. The player had to push back repeatedly before the advisor acknowledged it didn't actually know the cause.
...
| Uniform red across unrelated factors (injuries + food + sleep + unemployment simultaneously) | Trigger "check environmental conditions first" response rather than individual factor analysis |
