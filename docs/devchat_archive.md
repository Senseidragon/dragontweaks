...
- **Design Doc:** assistantmod_designdoc.md (in repo)
...
- ~~Should role slots be expandable via a new building?~~ → **Resolved:** No building. Slot cap is `maxRoleSlots` in `dragontweaks-common.toml` (default 3, range 1–8)
...
### Session 17 — Citizen Identification in Proactive Observations
**Date:** 2026-04-19

#### Problem
ObservationTicker was passing raw entity type strings to the LLM, allowing 
the model to invent citizen names, genders, and backstories. A MineColonies 
citizen triggered an observation that produced "Elara" — a name that does 
not exist in the colony.

#### Fix
ObservationTicker now checks each detected entity against the MineColonies 
API before building the observation prompt:
- If entity is a MineColonies citizen: query actual name and gender from 
  ICitizenData, pass both to LLM prompt
- If entity is not a citizen: pass entity type only, with explicit 
  instruction not to invent a name

#### Result
All four colony citizens (Anna I. Gill, Karsyn L. Alicock, Nikolas W. 
Collard, Reyansh M. Hammer) correctly identified by name and gender in 
proactive observations. No hallucinated names in any output.

#### Known limitation remaining
Citizen job/role descriptions are still invented by the LLM — citizen 
job data is not yet passed to the prompt. Acceptable until Phase 4.

*Last updated: Session 18 complete (2026-04-19)*

---

### Known Limitation — NPC Role Cross-Awareness
**Logged:** 2026-04-19

NPCs have no knowledge of other NPCs' assigned roles. If a player asks
"which of you is my advisor?" all NPCs in range respond independently
and most will be wrong or guess.

Current behavior: each NPC only knows its own role from its system prompt.
No shared awareness exists between NPC entities.

Resolution options (deferred):
- Inject a summary of all active NPC roles into each NPC's system prompt
  at query time ("Other assistants present: Charles (advisor), David (scout)")
- Restrict multi-NPC response to named addressing only — unnamed questions
  go to nearest NPC only
- Some combination of both

Priority: Low — becomes more important once real role assignment is live.
Revisit during Phase 1 role assignment design.
...
---
### Design Decisions Log — 2026-04-23
**Status: Pre-implementation. These decisions apply starting from Session 20 (Role Assignment UI).**

#### Decision 1 — Proximity Threshold for Command Detection

- **Value:** 10 blocks
- **Shape:** Cylindrical — XZ radius of 10 blocks, Y tolerance ±5 blocks independently
- **Rationale:** Spherical detection fails in multi-story MineColonies buildings where player may be directly above NPC. Cylindrical detection handles vertical separation naturally.
- **Implementation:** Add `COMMAND_RADIUS` to `Config.java` as `ConfigValue<Integer>`, default `10`. Never hardcode this value anywhere in the codebase. All proximity checks read from config at runtime.
- **Status:** ✅ Closed

#### Decision 2 — Role Slot Expansion

- **Mechanism:** Tied to Town Hall level. No separate building (Steward's Office / Command Post concept dropped).
- **Progression:**
  - Default (any TH level): 3 slots
  - TH Level 3: 4 slots
  - TH Level 4: 5 slots
  - TH Level 5: 6 slots
- **Hard cap:** 6 slots regardless of any other factor
- **Implementation — BREAKING CHANGE vs Session 19 spec:** `MAX_SLOTS = 3` defined as a constant in `RoleAssignmentData.java` is **superseded**. This must not be a hardcoded constant. Slot limit must be queried dynamically at runtime by finding the player's colony via MineColonies API, locating the Town Hall building, and reading its current level. The slot counter in `RoleAssignmentScreen` ("X / Y slots used") must reflect the dynamic max, not a hardcoded value.
- **Status:** ✅ Closed

#### Decision 3 — Scout Patrol Model

- **Model:** Periodic detection sweep. Not physical pathfinding. No waypoints.
- **Rationale:** Scout uses a shadow entity with no physical form. Pathfinding and waypoint systems are unnecessary complexity. The "patrol" is a configurable scan — the shadow entity samples points and runs AABB mob detection checks on a timer.
- **Implementation:** Scan radius and scan interval are both configurable values in `Config.java`. No navigation goal stack required. Simplifies Phase 3 considerably.
- **Status:** ✅ Closed

#### LLM Backend Update (carried forward — confirm before Session 21 code begins)

Per prior session note: Ollama has been replaced by OpenRouter.
- Endpoint: `https://openrouter.ai/api/v1/chat/completions`
- Model: `google/gemma-4-26b-a4b-it`
- Auth: `Authorization: Bearer <key>` — key stored in `Config.java` only, never hardcoded, never committed
- Request body uses OpenAI messages array format: `[{"role":"system","content":"..."},{"role":"user","content":"..."}]`
- `max_tokens: 100` replaces `num_predict: 100` — cap unchanged
- `stream: false` — unchanged
- `"reasoning": {"effort": "none"}` to disable thinking mode (validate in-game; fallback: `{"enabled": false}`)
- All references to `OllamaClient` in prior session notes should be read as `OpenRouterClient`

*Last updated: Session 21 design decisions (2026-04-23)*
...
#### Decision 1 — Proximity Threshold for Command Detection
**Design: CLOSED. Implementation: PENDING — not yet in codebase.**

- **Value:** 10 blocks
- **Shape:** Cylindrical — XZ radius of 10 blocks, Y tolerance ±5 blocks independently
- **Rationale:** Spherical detection fails in multi-story MineColonies buildings where the player may be directly above the NPC. Cylindrical detection handles vertical separation naturally.
- **What to build:** Add `COMMAND_RADIUS` to `Config.java` as `ConfigValue<Integer>`, default `10`. Never hardcode this value anywhere. All proximity checks read from config at runtime.

#### Decision 2 — Role Slot Expansion
**Design: CLOSED. Implementation: PENDING — neither `RoleAssignmentData.java` nor `RoleAssignmentScreen.java` exist yet. Build them fresh per this spec.**

- **Mechanism:** Tied to Town Hall level. No separate building. Steward's Office / Command Post concept is permanently dropped — do not reference it.
- **Slot progression:**
  - Default (any TH level below 3): 3 slots
  - TH Level 3: 4 slots
  - TH Level 4: 5 slots
  - TH Level 5: 6 slots
- **Hard cap:** 6 slots regardless of any other factor
- **Critical:** Do not use a hardcoded `MAX_SLOTS` constant. Slot limit must be queried dynamically at runtime — find the player's colony via MineColonies API, locate the Town Hall building, read its current level, return the corresponding slot count from the progression table above.
- **`RoleAssignmentScreen`** slot counter ("X / Y slots used") must use this dynamic value, not a constant.

#### Decision 3 — Scout Patrol Model
**Design: CLOSED. Implementation: PENDING — applies to Phase 3, not current session.**

- **Model:** Periodic detection sweep only. No pathfinding. No waypoints. No navigation goal stack.
- **Rationale:** The Scout uses a shadow entity with no physical form. The "patrol" is an AABB mob detection check fired on a timer at sampled points around the colony.
- **What to build (Phase 3):** Scan radius and scan interval as configurable values in `Config.java`. Nothing else.

#### LLM Backend — COMPLETE. NO ACTION REQUIRED.
`OllamaClient.java` has been replaced by `LLMClient.java`. OpenRouter is live and validated in-game. Do not rename, replace, or migrate anything. Do not look for `OllamaClient.java` — it is gone.

Specs for reference only:
- Endpoint: `https://openrouter.ai/api/v1/chat/completions`
- Model: `google/gemma-4-26b-a4b-it`
- Auth: `Authorization: Bearer <key>` — key in `.env` only, never hardcoded, never committed
- Request body: OpenAI messages array format
- `max_tokens: 100` — unchanged
- `stream: false` — unchanged
- `"reasoning": {"effort": "none"}` to disable thinking mode

*Last updated: Design decisions log 2026-04-23*